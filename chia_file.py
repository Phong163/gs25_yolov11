...
# Kiểm tra file nhãn tương ứng, xóa ảnh nếu không có nhãn và tạo danh sách hợp lệ
valid_image_files = []
for img_file in image_files:
    ...
    if os.path.exists(label_path):
        ...
        print(f"⚠️ Không tìm thấy file nhãn cho ảnh: {img_file}. Đang xóa ảnh...")
        try:
            os.remove(img_path)
            print(f"🗑️ Đã xóa ảnh: {img_file}")
        except Exception as e:
            print(f"❌ Lỗi khi xóa ảnh {img_file}: {e}")
